...
node_t *new_node(char *text) {
    ...
    // created new -- value to store the text
    temp->value = strdup(text);
    temp->next = NULL;
...
// created this new method to help with the sorting
node_t *add_alpha_order(node_t * list, node_t *new) {
    node_t *prev = NULL;
    node_t *curr = NULL;
    ...
    for (curr = list; curr != NULL; curr = curr->next) {
        if (strcmp(new->value, curr->value) > 0) {
            prev = curr;
        } else {
            break;
        }
